...
export const MockPaymentDialog = ({
  open,
  intent,
  onClose,
  onConfirm,
  ...
  options,
  loading = false,
  confirmLabel = 'Simuler le paiement',
  cancelLabel = 'Annuler',
  selectedOptionId = null,
  emptyMessage = 'Aucune option disponible pour le moment.',
  confirmDisabled = false
}: MockPaymentDialogProps) => {
  const content = useMemo(() => contentByIntent[intent], [intent])
  const resolvedTitle = title ?? content.title
  const resolvedDescription = description ?? content.description
  const resolvedOptions = options ?? content.options
  const [selectedOption, setSelectedOption] = useState(() => {
    if (selectedOptionId && resolvedOptions.some(option => option.id === selectedOptionId)) {
  ...
  useEffect(() => {
    ...
    if (resolvedOptions.length > 0) {
      setSelectedOption(resolvedOptions[0].id)
    } else {
      setSelectedOption('')
    }
  }, [open, resolvedOptions, selectedOptionId])

  if (!open) return null

  const handleConfirm = () => {
    if (loading || confirmDisabled || !selectedOption) return
    const option = resolvedOptions.find(item => item.id === selectedOption)
    if (!option) return
    onConfirm?.(option)
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur">
      <div className="w-full max-w-xl rounded-2xl border border-slate-700 bg-slate-900/95 p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-xl font-semibold text-slate-100">{resolvedTitle}</h2>
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <button
            ...
            disabled={loading || confirmDisabled || !selectedOption}
