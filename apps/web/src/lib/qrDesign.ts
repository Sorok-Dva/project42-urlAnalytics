...
const moduleMap: Record<QrDesign['modules'], string> = {
  'dots-classic': 'dots',
...
const pilotBorderMap: Record<QrDesign['pilotBorder'], string> = {
  square: 'square',
  dot: 'dot',
  rounded: 'extra-rounded'
}

const pilotCenterMap: Record<QrDesign['pilotCenter'], string> = {
  dot: 'dot',
  rounded: 'extra-rounded',
  square: 'square'
}
...
const buildLogoValue = (design: QrDesign) => {
  if (design.logo.type === 'p42') return LOGO_ASSETS.p42
  if (design.logo.type === 'app') return LOGO_ASSETS.app
  if (design.logo.type === 'custom') return design.logo.value ?? null
  return null
}
...
export const renderQrToElement = (
  element: HTMLElement,
  design: QrDesign,
  data: string,
  size = 320
...
export const designEquals = (a: QrDesign, b: QrDesign) => {
  return JSON.stringify(a) === JSON.stringify(b)
}

export const sanitizeDesign = (design?: Partial<QrDesign> | null): QrDesign => {
  return {
    ...DEFAULT_QR_DESIGN,
    ...design,
    logo: {
